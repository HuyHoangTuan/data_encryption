...
from src.modules.utils.psychoacoustic import *
from src.modules.utils.quantizer import *
from src.modules.utils.rle import *
from src.modules.utils.huffman import *
...
def MP3codec(wavin, h, M, N):
    ...
    xhat = MP3decod(total_stream, total_frame_symbol_prob, stream_breakpoints, frame_symbol_prob_breakpoints, total_sc,
                    total_B, h, M, N)
...
def MP3cod(wavin, h, M, N):
    ...
    return total_stream, total_frame_symbol_prob, stream_breakpoints.astype(
        np.int64), frame_symbol_prob_breakpoints.astype(np.int64), total_sc, total_B.astype(np.int64)


def MP3decod(total_stream, total_frame_symbol_prob, stream_breakpoints, frame_symbol_prob_breakpoints, total_sc,
             total_B, h, M, N):
    Ytot = np.empty((0, M))
    ...
    for i in np.arange(len(stream_breakpoints) - 1):
        ...
        frame_symbol_prob = total_frame_symbol_prob[
                            frame_symbol_prob_breakpoints[i]:frame_symbol_prob_breakpoints[i + 1], :]
        sc = total_sc[i, :]
        B = total_B[i]
